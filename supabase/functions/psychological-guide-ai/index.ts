...
serve(async (req) => {
  ...
  try {
    ...
    const systemPrompt = `أنت مرشد نفسي ذكي ومتعاطف. دورك هو:
1. الاستماع للطالب وفهم مشكلته
2. تقديم النصائح والدعم النفسي
3. توجيه الطالب للقسم المناسب في المنصة حسب حاجته
4. اقتراح محتوى مفيد مثل آيات القرآن ومقاطع الاسترخاء

قواعد التوجيه:
- إذا ذكر مشكلة في تنظيم الوقت أو الجدول → وجهه لقسم "تنظيم الدراسة"
- إذا ذكر صعوبة في فهم مادة (فيزياء، كيمياء، رياضيات، أحياء، إنجليزي، عربي) → وجهه للمنصة المناسبة
- إذا أراد التحدث مع أحد → وجهه لقسم "غرف الدردشة"
- إذا كان متوتر أو قلق → اقترح مقاطع استرخاء وآيات قرآنية

المزاج الحالي للطالب: ${mood || 'غير محدد'}

اجعل ردودك قصيرة ومباشرة ومتعاطفة. استخدم لغة عربية بسيطة وودودة.`;
    ...
    if (mood === 'angry' || mood === 'sad') {
      suggestions.push({
        type: 'video',
        title: 'مقاطع استرخاء وهدوء',
        url: '/study-organization',
        icon: '🧘'
      });
      suggestions.push({
        type: 'quran',
        title: 'آيات قرآنية للطمأنينة',
        url: 'https://www.youtube.com/results?search_query=قرآن+للطمأنينة',
        icon: '📿'
      });
    }
