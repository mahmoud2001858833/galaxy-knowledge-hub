...
async function fetchTranscriptViaInvidious(videoId: string, preferredLang?: string): Promise<{ lang: string; segments: Segment[] } | null> {
  ...
  for (const base of INVIDIOUS_INSTANCES) {
    try {
      const r = await fetchWithTimeout(`${base}/api/v1/captions/${videoId}`, 6000, { headers: { "User-Agent": "Mozilla/5.0" } });
      if (!r.ok) continue;
      ...
      const cr = await fetchWithTimeout(capUrl, 6000, { headers: { "User-Agent": "Mozilla/5.0" } });
      if (!cr.ok) continue;
...
async function fetchTranscriptViaPiped(videoId: string, preferredLang?: string): Promise<{ lang: string; segments: Segment[] } | null> {
  for (const base of PIPED_INSTANCES) {
    try {
      const r = await fetchWithTimeout(`${base}/streams/${videoId}`, 6000, { headers: { "User-Agent": "Mozilla/5.0" } });
      if (!r.ok) continue;
      const d = await r.json();
      ...
      const pick =
        ...
        subs[0];
      if (!pick?.url) continue;
      const segments = await fetchSubtitleUrl(pick.url);
