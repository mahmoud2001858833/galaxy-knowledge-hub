  ...
  return <div className="min-h-screen flex flex-col text-right bg-gradient-to-b from-indigo-950 via-purple-900 to-black" dir="rtl">
      ...
      <div className="fixed inset-0 z-0 pointer-events-none">
        ...
        <div className="absolute top-0 left-0 w-full h-full">
          ...
          <div className="absolute top-2/3 left-1/4 w-64 h-64 rounded-full bg-cyan-500/10 blur-3xl animate-pulse" style={{
          animationDelay: '2s'
        }} />
        </div>
      </div>
      ...
      <main className="flex-1 container mx-auto px-4 py-6 relative z-10 flex flex-col max-w-6xl">
        ...
        y: 0
            ...
            <AnimatePresence>
              ...
              y: -20
                  ...
                  <div className={`max-w-[80%] p-4 rounded-2xl relative ${message.type === 'user' ? 'bg-indigo-600/30 border border-indigo-500/50 text-white' : message.type === 'system' ? 'bg-purple-900/40 border border-purple-500/50 text-purple-100' : 'bg-gray-800/50 border border-gray-600/50 text-gray-100'}`}>
                    ...
                    {message.hasImage && <div className="mt-2">
          ...
          {/* Image Preview */}
          {imagePreview && <motion.div initial={{
          opacity: 0,
          scale: 0.8
        }} animate={{
          opacity: 1,
          scale: 1
              ...
              <img src={imagePreview} alt="Preview" className="max-w-32 max-h-32 rounded-lg border-2 border-indigo-500/50" />
              <button onClick={removeImage} className="absolute -top-2 -right-2 w-6 h-6 bg-red-500 rounded-full flex items-center justify-center text-white text-xs hover:bg-red-600">
                <X className="w-3 h-3" />
              </button>
            </motion.div>}
