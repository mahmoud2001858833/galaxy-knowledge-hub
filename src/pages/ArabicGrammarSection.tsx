...
import { Book, Scroll, ArrowRight } from 'lucide-react';

const ArabicGrammarSection = () => {
  const navigate = useNavigate();

  const grammarTools = [
    { id: 'basics', title: 'أساسيات النحو', icon: Book, description: 'تعلم القواعد الأساسية', path: '/arabic-platform/grammar/basics' },
    { id: 'library', title: 'مكتبة القواعد', icon: Scroll, description: 'مراجع شاملة', path: '/arabic-platform/grammar/library' },
  ];
        ...
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {grammarTools.map((tool, idx) => (
              ...
              >
                <tool.icon className="w-12 h-12 text-blue-400 mb-4 group-hover:scale-110 transition-transform" />
                <h3 className="text-2xl font-bold text-blue-300 mb-2">{tool.title}</h3>
                <p className="text-blue-100/70">{tool.description}</p>
              </motion.div>
            ))}
          </div>
        </motion.div>
