...
import SignGlyph from '@/features/sign-language/SignGlyph';
import { getStrings, signSystemForLang } from '@/features/sign-language/i18n';
...
const SignDictionary: React.FC = () => {
  ...
  const [signSystemTouched, setSignSystemTouched] = useState(false);
  ...
  const t = useMemo(() => getStrings(lang.code), [lang.code]);
  const dir = t.rtl ? 'rtl' : 'ltr';

  // Load / fetch titles when language changes; also auto-switch sign system unless user picked one.
  useEffect(() => {
    let cancelled = false;
    setTitles(loadTitles(lang.code));
    setVisibleCount(60);
    if (!signSystemTouched) setSignSystem(signSystemForLang(lang.code));
    ...
    fetchTitlesIfNeeded(lang.code, lang.name, (done, total) => {
      ...
      .then((map) => { if (!cancelled) { setTitles(map); setTranslatingProgress(null); } })
      .catch((e) => {
        if (cancelled) return;
  ...
  }, [lang, signSystemTouched]);

  useEffect(() => { if (selected === null) setFavs(getFavorites()); }, [selected]);
  ...
  return (
    <div className="px-4 sm:px-6 pt-12 pb-16 max-w-7xl mx-auto" dir={dir}>
      <div className="mb-6">
        <h1 className="text-3xl sm:text-4xl font-extrabold text-[hsl(var(--damij-primary))] mb-2 flex items-center gap-3">
          <Globe className="w-9 h-9" /> {t.title}
        </h1>
        <p className="text-slate-600">
          {t.subtitle(DICTIONARY.length, CATEGORIES.length, SPOKEN_LANGUAGES.length, SIGN_SYSTEMS.length)}
        </p>
      </div>
      ...
      <div className="grid lg:grid-cols-2 gap-4 mb-6">
        <div className="bg-white rounded-2xl border border-[hsl(var(--damij-primary))]/15 p-4">
          <label className="text-sm font-bold text-[hsl(var(--damij-primary))] flex items-center gap-2 mb-2">
            <Hand className="w-4 h-4" /> {t.signSystem}
          </label>
          <select
            value={signSystem}
            onChange={(e) => { setSignSystem(e.target.value); setSignSystemTouched(true); }}
        ...
        <div className="bg-white rounded-2xl border border-[hsl(var(--damij-primary))]/15 p-4">
          <label className="text-sm font-bold text-[hsl(var(--damij-primary))] flex items-center justify-between mb-2">
            <span className="flex items-center gap-2"><Globe className="w-4 h-4" /> {t.displayLanguage}</span>
            <span className="text-xs font-normal text-slate-500">{t.selected}: {lang.flag} {lang.nativeName}</span>
          </label>
          <input
            value={langSearch}
          ...
          {translatingProgress && (
            <div className="mt-2 text-xs text-[hsl(var(--damij-primary))] flex items-center gap-2">
              ...
              {t.translating(lang.nativeName)} ({translatingProgress.done}/{translatingProgress.total})
            </div>
          )}
        </div>
      </div>
      ...
      <div className="relative mb-5">
        <Search className={`absolute ${t.rtl ? 'right-4' : 'left-4'} top-1/2 -translate-y-1/2 w-5 h-5 text-[hsl(var(--damij-primary))]`} />
        <input
          value={query}
          onChange={(e) => setQuery(e.target.value)}
          placeholder={t.searchPlaceholder(DICTIONARY.length, lang.nativeName)}
          className={`w-full p-4 ${t.rtl ? 'pr-12' : 'pl-12'} rounded-2xl border-2 border-[hsl(var(--damij-primary))]/20 bg-white text-lg shadow-sm focus:outline-none focus:border-[hsl(var(--damij-primary))]/50`}
          dir="auto"
        />
      </div>
      ...
      <div className="grid lg:grid-cols-[260px_1fr] gap-6">
        <aside className="bg-white rounded-2xl border border-[hsl(var(--damij-primary))]/15 p-3 h-fit max-h-[600px] overflow-y-auto sticky top-4">
          <h3 className="text-sm font-bold text-[hsl(var(--damij-primary))] mb-2 px-2">{t.categories}</h3>
          <div className="space-y-1">
            <button
              onClick={() => setCategory('all')}
              className={`w-full ${t.rtl ? 'text-right' : 'text-left'} px-3 py-2 rounded-xl text-sm flex items-center justify-between transition-colors ${
            ...
            >
              <span><Star className="w-3.5 h-3.5 inline mb-0.5" /> {t.favorites}</span>
        ...
        <main>
          {filtered.length === 0 ? (
            ...
            <>
              <div className="text-sm text-slate-500 mb-3">{t.results(filtered.length)}</div>
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 xl:grid-cols-5 gap-3">
                {visible.map((w) => {
                  const translated = titles[w.ar];
                  const isFav = favs.includes(w.id);
                  const display = !lang.code.startsWith('ar') && translated ? translated : w.ar;
                  return (
                    <motion.button
                      key={w.id + signSystem}
                    ...
                    >
                      {isFav && <Star className={`w-3.5 h-3.5 absolute top-2 ${t.rtl ? 'left-2' : 'right-2'} fill-yellow-400 text-yellow-400`} />}
                      <SignGlyph word={w} signSystem={signSystem} size={70} className="mb-2" />
                      <div className="font-bold text-[hsl(var(--damij-primary))] text-sm leading-tight" dir="auto">{display}</div>
                      {!lang.code.startsWith('ar') && translated && (
                        <div className="text-[10px] text-slate-400 mt-0.5 line-clamp-1" dir="rtl">{w.ar}</div>
                      )}
                    </motion.button>
                  );
                })}
              ...
              {visibleCount < filtered.length && (
                <div className="mt-6 text-center">
                  ...
                  >
                    <Sparkles className="w-4 h-4" /> {t.loadMore(filtered.length - visibleCount)}
                  </button>
                </div>
              )}
            </>
