...
import GradeElevenBiologyFirstSemester from './GradeElevenBiologyFirstSemester';

interface GradeElevenBiologyProps {
  onBack: () => void;
}

const GradeElevenBiology = ({ onBack }: GradeElevenBiologyProps) => {
  ...
  const semesters = [
    {
      ...
      description: 'الخلية والوراثة، التطور والتنوع الحيوي'
    },
    {
      id: 'second',
      title: 'الفصل الثاني',
  ...
  if (selectedSemester === 'first') {
    return <GradeElevenBiologyFirstSemester onBack={() => setSelectedSemester(null)} />;
  }
  ...
  return (
    <div className="space-y-6 relative">
      ...
      <div className="absolute top-0 left-4 z-10">
        <div className="bg-gradient-to-r from-green-600/20 to-emerald-600/20 backdrop-blur-sm border border-green-500/30 rounded-lg px-3 py-2">
          <p className="text-white/80 text-sm font-medium">فيديوهات تعليمية متميزة</p>
        </div>
      </div>
...
export default GradeElevenBiology;
